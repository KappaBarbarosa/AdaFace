...
from yolo_face.utils.google_utils import gsutil_getsize
from yolo_face.utils.metrics import fitness
from yolo_face.utils.torch_utils import init_torch_seeds
...
def get_crop(xyxy, im, gain=1.02, pad=10, square=False, BGR=False):
    xyxy = torch.tensor(xyxy).view(-1, 4)
    b = xyxy2xywh(xyxy)  # boxes
    if square:
        b[:, 2:] = b[:, 2:].max(1)[0].unsqueeze(1)  # attempt rectangle to square
